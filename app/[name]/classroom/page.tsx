...
import { getCoachBySlug, getCoachCourses } from '@/app/coachme/data/utils';
import { CourseCard } from '../components/course-card';
import { courseCategories } from '@/app/coachme/data/course-categories';
...
function ClassroomPage() {
    ...
    const allCourses = coach?.courseIds
        ? getCoachCourses(coach.id)
        : [];

    // Filter courses by category
    const filteredCourses = selectedCategory === 'all'
        ? allCourses
        : allCourses.filter(course => course.category.toLowerCase() === selectedCategory.toLowerCase());
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {filteredCourses.map((course) => (
                        <CourseCard
                            key={course.id}
                            courseItem={course}
                            onClick={() => router.push(`/coachme/${coachSlug}/classroom/${course.slug}`)}
                        />
                    ))}
                </div>
            ) : (
